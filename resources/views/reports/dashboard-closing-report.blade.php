...
@section('report-content')
    @php
        /*
        ════════════════════════════════════════════════════════════════════
        DATA CONTRACT  (from FinancialTransactionService::getPackageGroupPaymentSummary)
        ════════════════════════════════════════════════════════════════════

        $body = [
            'mode'             => 'monthly' | 'daily',
            'period_label'     => 'Monthly',
            'date_range_label' => '1 March 2026 – 31 March 2026',
            'package'          => ['id'=>1,'package_number'=>'KTG-1','name'=>'...'] | null,
            'categories'       => ['umrah_packages'=>'Umrah Packages', ...],
            'payment_methods'  => ['cash','nets','visa','master','paynow'],
            'rows'             => [
                [
                    'date_sort'      => '2026-03-03',
                    'date'           => '3 March 2026',
                    'day_name'       => 'Monday',
                    'umrah_packages' => 550.00,
                    ...
                    'cash'           => 0.00,
                    'nets'           => 0.00,
                    'total_sales'    => 550.00,
        ...
        */
        ...
        /* Sum a subset of rows into a totals array */
        $sumRows = function (array $subset) use ($catKeys, $methodKeys): array {
            $t = array_fill_keys(array_merge($catKeys, $methodKeys, ['total_sales']), 0.0);
            foreach ($subset as $r) {
                foreach (array_merge($catKeys, $methodKeys, ['total_sales']) as $k) {
                    $t[$k] += (float) ($r[$k] ?? 0);
                }
            }
            return $t;
        ...
        if ($mode === 'monthly') {
            /*
             * Monthly: group rows by week (day 1–7 = Week 1, 8–14 = Week 2, …).
             * Fill in empty-day stubs for every date in the period so the
             * table always shows a full calendar month (matching sample PDF).
             */
            ...
            /* Index existing rows by date_sort */
            $rowIndex = [];
            foreach ($rows as $r) {
                $rowIndex[$r['date_sort']] = $r;
            }

            /* Build all dates + weeks */
            $weekGroups = []; // ['Week N' => [rows…]]
            ...
            while ($cursor->lte($rangeEnd)) {
                ...
                if (!isset($weekGroups[$weekLabel])) {
                    $weekGroups[$weekLabel] = [];
                }

                if (isset($rowIndex[$dateKey])) {
                    $weekGroups[$weekLabel][] = $rowIndex[$dateKey];
                } else {
                    /* Empty stub */
                    $stub = [
                        'date_sort' => $dateKey,
                        'date' => $cursor->translatedFormat('d F Y'),
                        'day_name' => $cursor->translatedFormat('l'),
                        'total_sales' => 0.0,
                        '__empty' => true,
                    ];
                    foreach (array_merge($catKeys, $methodKeys) as $k) {
                        $stub[$k] = 0.0;
                    }
                    $weekGroups[$weekLabel][] = $stub;
                }
            ...
            $plan = [];
            $grandTotal = array_fill_keys(array_merge($catKeys, $methodKeys, ['total_sales']), 0.0);

            foreach ($weekGroups as $weekLabel => $weekRows) {
                $weekTotal = $sumRows($weekRows);
                foreach (array_keys($grandTotal) as $k) {
                    $grandTotal[$k] += $weekTotal[$k];
                }
                $plan[] = [
                    'label' => $weekLabel,
                    'rows' => $weekRows,
            ...
            /*
             * Daily: flat list of date rows, no week grouping.
             * Fill in every date in the range (same empty-stub logic).
             */
            $rangeStart = \Carbon\Carbon::parse($report['start_date'] ?? now()->startOfDay());
            $rangeEnd = \Carbon\Carbon::parse($report['end_date'] ?? now()->endOfDay());
            $rowIndex = [];
            foreach ($rows as $r) {
                $rowIndex[$r['date_sort']] = $r;
            }
            ...
            $grandTotal = $sumRows($allDailyRows);
            $plan = [['label' => null, 'rows' => $allDailyRows, 'total' => $grandTotal]];
        }

        $isMonthly = $mode === 'monthly';

        /* ── Column count for colspan calculations ───────────────── */
        // Date | [cats…] | [methods…] | Total Sales
        $totalCols = 1 + count($catKeys) + count($methodKeys) + 1;
    ...
    <table class="section-table">
        ...
        <tbody>
            ...
            @else
                {{-- ══════════════════════════════════════════════════════
                 DAILY: flat date rows (no week grouping)
                 ══════════════════════════════════════════════════════ --}}

                @foreach ($plan[0]['rows'] as $row)
                    @php $isEmpty = !empty($row['__empty']); @endphp
                    <tr class="{{ $isEmpty ? 'row-empty' : '' }}">
                        <td class="td-date">
                            {{ $row['date'] }}
