...
export function EmployerFeedbackSection({
    data,
    setData,
    isView,
}: EmployerFeedbackSectionProps) {
    const feedbacks = (data.employer_feedback ||
        []) as EmployerFeedbackRecord[];
    ...
    const addRow = () => {
        ...
        if (hasIncompleteRecord) {
            toast.warning(
                'Please complete both employer and feedback before adding a new one',
            );
            return;
        }
    ...
    const removeRow = (index: number) => {
        ...
        if (displayFeedbacks.length <= 2) {
            toast.warning(
                'At least 2 employer feedback records must be maintained',
            );
            return;
        }
        ...
        if (hasAnyValue) {
            if (
                !confirm(
                    'Are you sure you want to remove this employer feedback record?',
                )
            ) {
                return;
            }
    ...
    const hasError = (index: number, field: string) => {
        ...
        if (
            field === 'feedback' &&
            (!row.feedback || row.feedback.trim() === '')
        )
            return true;
        return false;
    };
    ...
    return (
        <section className="space-y-4">
            ...
            <p className="text-base text-muted-foreground">
                Feedback was/was not obtained by the EA from the previous
                employers. If feedback was obtained (attach testimonial if
                        ...
                        <div className="md:col-span-9">
                            ...
                            {hasError(index, 'feedback') && (
                                <p className="mt-1 text-sm text-red-500">
                                    Feedback is required when employer is
                                    provided
                                </p>
                            )}
                        </div>
