        ...
        <aside className="sidebar-lhs-parent sticky top-0 flex h-[calc(100vh-4rem)] w-[300px] shrink-0 flex-col overflow-hidden border-r border-sidebar-border/70 bg-white dark:bg-slate-900/80">
            ...
            <div className="border-b border-sidebar-border/70 px-4 py-4">
                <div className="flex items-center gap-2">
                    <BookOpen className="h-5 w-5 text-primary dark:text-primary/80" />
            ...
            <div className="border-b border-sidebar-border/70 px-3 py-3">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full rounded-lg border border-sidebar-border/70 bg-slate-50 py-1.5 pr-3 pl-8 text-xs text-foreground transition-colors placeholder:text-muted-foreground focus:border-primary/30 focus:bg-white focus:ring-1 focus:ring-primary/20 focus:outline-none dark:bg-slate-800 dark:focus:border-primary/70 dark:focus:bg-slate-900 dark:focus:ring-primary/20"
                    />
                </div>
            </div>
            ...
            <nav className="flex-1 overflow-y-auto px-2 py-2 [scrollbar-gutter:stable]">
                ...
                {filteredGroups.map((group) => {
                    ...
                    return (
                        <div
                            key={group.menu}
                            className={`mb-0.5 rounded-lg transition-colors duration-150 ease-out ${
                        ...
                        >
                                ...
                                {procedures.length > 0 && (
                                    <button
                                        ...
                                        className={`flex w-9 shrink-0 items-center justify-center rounded-r-lg transition-colors duration-150 ease-out ${
                                            isActiveModule &&
                                            !activeProcedureSlug
                                                ? 'text-primary hover:bg-primary/10 dark:text-primary/80 dark:hover:bg-primary/15'
                            ...
                            {isExpanded && procedures.length > 0 && (
                                <div className="mt-0.5 ml-4 space-y-0.5 border-l border-sidebar-border/50 pl-2">
                                    {procedures.map((proc, idx) => {
                                        const pSlug = slugify(proc.name);
                                        const isActive =
                                            isActiveModule &&
                                            activeProcedureSlug === pSlug;
                                        ...
                                        return (
                                            <Link
                                                ...
                                                className={`flex w-full items-center gap-1.5 border-l-2 px-2 py-1.5 text-left text-[11px] transition-colors duration-150 ease-out ${
                                                    isActive
                                                        ? 'rounded-r-md border-primary bg-primary/5 font-bold text-primary dark:bg-primary/10 dark:text-primary/80'
            ...
            <div className="min-h-[calc(100vh-4rem)]">
                ...
                {isDetailView && !isHomeView && (
                    ...
                    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
                        ...
                        <div
                            id="doc-content-area"
                            className="resource-content-wrap min-w-0 flex-1 overflow-y-auto bg-slate-50/60 dark:bg-slate-950"
                        >
                            ...
                            {nav.view === 'procedure' &&
                                nav.selectedModule &&
                                nav.selectedProcedure !== null && (
                                    <ProcedureDetailView
                                        documentation={documentation}
                                        moduleGroup={nav.selectedModule}
                                        procedureIndex={nav.selectedProcedure}
                                        onBackToModule={() =>
                                            nav.goToModule(nav.selectedModule!)
                                        }
