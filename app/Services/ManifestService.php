...
class ManifestService
{
    ...
    public function getForEditShow($id): array
    {
        $manifest = $this->scopedManifestQuery()->with([
            ...
            'members.confirmationMember.customer.files',
        ...
        $members = $manifest->members
            ...
            ->map(function ($member, $index) use ($manifest) {
                ...
                $user = $customer?->user;
                $customerDocuments = $customer?->relationLoaded('files')
                    ? $customer->files->keyBy('field')
                    : collect();
        ...
        // dd(collect($members)->map(fn ($member) => [
        //     'customer_name' => $member['customer_name'],
        //     'package_price' => $member['package_price'],
        //     'discount' => $member['discount'],
        //     'deposit_payment' => $member['deposit_payment'],
        //     'second_payment' => $member['second_payment'],
        //     'third_payment' => $member['third_payment'],
        ...
        return CustomerConfirmation::with([
            ...
            ->map(function (CustomerConfirmation $confirmation) {
                ...
                return [
                    ...
                    'members' => $activeMembers->map(function ($member) use ($confirmation) {
                        $customer = $member->customer;
                        $user = $customer?->user;
        ...
        $nonOfficialMembers = ManifestMember::query()
            ...
            ->where(function ($query): void {
                $query
                    ...
                    ->orWhereHas('confirmationMember', function ($confirmationQuery): void {
                        $confirmationQuery
                            ->whereNull('status')
    ...
    private function buildMemberFinancialSnapshot(
        ...
        $balanceDue = round(max(
            $memberPayableAmount - $adjustedPaidAmount,
            0
        ), 2);
    ...
    private function areAllMemberInvoicesSettled(CustomerConfirmationMember $member): bool
    {
        ...
        foreach ($memberInvoices as $invoice) {
            ...
            $invoiceTotalAmount = max((float) ($invoice->amount ?? 0), 0.0);
            ...
            $paidAmount = $receiptTotal !== 0.0
                ? min($receiptTotal, $invoiceTotalAmount)
                : $fallbackPaidAmount;

            $outstandingAmount = round(max($invoiceTotalAmount - $paidAmount, 0), 2);
    ...
    private function syncManifestDocuments(Manifest $manifest, array $documents): void
    {
        ...
        $identityDocumentRows = $this->buildIdentityDocumentRowsForManifest($manifest);
        $rowsToPersist = [...$rowsToPersist, ...$identityDocumentRows];
        ...
        foreach ($existingManagedFiles as $existingFile) {
            if (! in_array($existingFile->file_path, $preservedPaths, true) && $existingFile->file_path) {
                $normalizedPath = ltrim((string) $existingFile->file_path, '/');

                if (str_starts_with($normalizedPath, 'manifests/')) {
    ...
    private function syncIdentityDocumentsFromMembers(Manifest $manifest): void
    {
        $identityDocumentRows = $this->buildIdentityDocumentRowsForManifest($manifest);

        $manifest->files()
            ->whereIn('field', ['passport', 'photo'])
            ->delete();

        foreach ($identityDocumentRows as $row) {
            $manifest->files()->create($row);
        }
    }
    ...
    private function buildIdentityDocumentRowsForManifest(Manifest $manifest): array
    {
        ...
        return $manifestMembers
            ->values()
            ->flatMap(function (ManifestMember $member, int $index): array {
                $rows = [];
